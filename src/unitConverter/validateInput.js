...
const validateInput = (userInputString) => {
    ...
    if (!(/^\d+$/.test(number)) || parseInt(number) < 0){
        throw new Error(`Invalid command: ${number} is not a valid number, please enter a nonnegative integer ot a nonnegative floating-point number in digit form, symbolic representation of numbers is not supported. Commands should take the form: babylon -c "<number> <unit1> in <unit2>"`)
